...
function repairResultIfNeeded() {
  for (let attempt = 1; attempt <= resultRepairAttempts; attempt += 1) {
    ...
    const repairPrompt = [
      ...
      "Security repair rule: `route_security` must target exact issue/PR refs only. Never emit `route_security` for `cluster:<cluster_id>`. Do not turn a validator error into `route_security` unless the matching hydrated preflight item explicitly has `security_sensitive: true`, or that item is unavailable with a documented hydration failure. A preflight `security_sensitive: false` is not overridable by check names, comments, labels, or validator wording. If a security-sensitive ref is separately routed and the non-security cluster-level fix action still has a complete executable `fix_artifact`, keep or repair that fix action. Replace it with cluster-scoped `needs_human` using `target_kind: null` and `target_updated_at: null` only when the fix itself depends on the quarantined security decision or no complete executable fix artifact is safe.",
      "Fix artifact repair rule: never emit `build_fix_artifact`, `open_fix_pr`, or planned `fix_needed` as a placeholder, and never leave `fix_needed` by itself without a complete executable `fix_artifact`. A cluster-scoped fix action must target exactly `cluster:<cluster_id>` with no synthetic suffix or subcluster name, and a result may contain at most one `build_fix_artifact` action. If several unrelated repairs are viable, keep them as individual hydrated `fix_needed` actions and do not combine them into one artifact. If no complete executable `fix_artifact` is safe from the artifacts, use cluster-scoped `needs_human` instead.",
      "Blocked repair lineage rule: if a validator failure says an executable repair target has a blocked label, remove that ref from `candidate_fix`, `fix_artifact.linked_refs`, and `fix_artifact.source_prs`; keep the ref only as non-mutating `keep_related` evidence. If removing it means the artifact no longer has safe executable lineage or source proof, replace the fix action with cluster-scoped `needs_human`.",
