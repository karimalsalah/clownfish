...
- If any hydrated item is security-sensitive, quarantine that item with `route_security` and route it to central OpenClaw security triage. Do not mutate that item. Continue classifying unrelated non-security items, duplicate pairs, provider gaps, and ordinary bugs. Never emit cluster-scoped `route_security`; route each exact issue/PR ref that has the security signal.
- Emit `route_security` only for an exact hydrated item marked `security_sensitive: true`, or an unavailable item with a documented hydration failure. A preflight `security_sensitive: false` is authoritative for this run; check names, comments, labels, or validator wording cannot override it.
- If security-sensitive refs block only the quarantined ref, still emit `route_security` only for the exact issue/PR refs and keep a separate non-security cluster-level fix path moving when you can include a complete executable `fix_artifact`. For the cluster-level fix lane, emit a cluster-scoped `needs_human` action with `target: "cluster:<cluster_id>"`, `target_kind: null`, and `target_updated_at: null` only when the fix itself depends on the quarantined security decision or no complete executable fix artifact is safe; never emit cluster-scoped `route_security`.
...
- Emit `fix_needed`, `build_fix_artifact`, or `open_fix_pr` only when the job allows both `fix` and `raise_pr` and `allow_fix_pr` is true. If the job cannot open a fix PR, keep the live canonical issue/PR classified with `keep_canonical`/`keep_related`/`keep_independent` and use `needs_human` only for the exact follow-up decision that remains unresolved.
- If no viable canonical PR exists and the bug still appears real from the artifact, emit `fix_needed` plus `build_fix_artifact` only in a fix-enabled job. Do not build non-executable fix artifacts in close-only, dedupe-only, or no-fix jobs.
- Never emit `fix_needed` by itself as a placeholder. If you cannot provide a complete executable `fix_artifact` and matching cluster-scoped `build_fix_artifact`, use non-mutating classification actions or cluster-scoped `needs_human` with exact evidence instead.
...
- `validation_commands` must be executor-supported commands using the target repo's package manager. For `openclaw/openclaw`, `pnpm check:changed` is the hard local gate; focused tests may be listed as helpful context but the executor may normalize them to `pnpm check:changed` to avoid blocking on unrelated flaky main CI. Do not emit `npm run validate` because that script does not exist there. Do not include `Codex /review`: the executor runs it separately and records it in `merge_preflight`. Do not use ad hoc `pnpm exec tsx -e` probes; put those manual checks in action evidence instead. Do not include `scripts/pr prepare-*`, `scripts/pr merge-*`, or other PR lifecycle commands: the executor owns prep, push, and merge flow. Only `scripts/pr review-validate-artifacts <positive-pr-number>` is valid from that wrapper. Put manual browser checks and prose test plans in `pr_body`, `credit_notes`, or action evidence, not in `validation_commands`.
...
- Do not use `build_fix_artifact` as a placeholder for blocked/security-routed work. If no complete executable `fix_artifact` is safe, use cluster-scoped `needs_human` instead and keep security routing on the exact refs.
- Do not emit `build_fix_artifact`, `open_fix_pr`, or planned `fix_needed` when any executable repair target or lineage ref is blocked by a high-risk mutation label such as `merge-risk:*` or `clawsweeper:automerge`, unless the job is an adopted automerge repair that explicitly allows that source. This includes `candidate_fix`, `fix_artifact.linked_refs`, and `fix_artifact.source_prs`: if a useful contributor PR has a blocked label, keep it as `keep_related` evidence and use cluster-scoped `needs_human` instead of putting that PR in the executable fix artifact for credit.
