...
For fix artifacts, `validation_commands` must contain executor-supported commands only. Never include `Codex /review` there: the executor always runs and records that review separately. Do not use ad hoc `pnpm exec tsx -e` probes; use a focused test command plus `pnpm check:changed`. Put review requirements and manual probes in the action evidence or `pr_body`.
